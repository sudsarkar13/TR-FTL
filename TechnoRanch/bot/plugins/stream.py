...
async def login_handler(c: Client, m: Message):
    try:
        try:
            ag = await m.reply_text("Now send me password.\n\n If You don't know check the MY_PASS Variable in vercel \n\n(You can use /cancel command to cancel the process)")
                    ...
                    [
                        [
                            InlineKeyboardButton(
                                "ᴊᴏɪɴ ɴᴏᴡ", url=f"https://t.me/{Var.UPDATES_CHANNEL}")
                        ]
                    ]
                )
            )
...
<b>♻️ ᴛʜɪs ʟɪɴᴋ ɪs ᴘᴇʀᴍᴀɴᴇɴᴛ ᴀɴᴅ ᴡᴏɴ'ᴛ ɢᴇᴛs ᴇxᴘɪʀᴇᴅ ♻️</b>"""
        ...
        await m.reply_text(
            ...
            text=msg_text.format(get_name(log_msg), humanbytes(
                get_media_file_size(m)), online_link, stream_link),
